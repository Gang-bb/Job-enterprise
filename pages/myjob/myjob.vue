	...
	<view class="container999" @touchstart="refreshStart" @touchmove="refreshMove" @touchend="refreshEnd">
		...
		<view class='nav'>
			...
			<view class='searchInput999'>
				...
				<input class='input999' placeholder="输入工作标题搜索" v-model="key"></input>
				<view class="sousuo" hover-class="sousuo-click" hover-stay-time="100" @click="sousuo">
					搜索
				</view>
			</view>
			<!-- 导航栏 agents导航栏标题 -->
		...
		<swiper style="min-height: 100vh;" :current="currentTab" @change="swiperTab">
			<swiper-item v-for="(listItem,listIndex) in myList" :key="listIndex">
				...
				<view class='content'>
					<view class="item" v-for="(item,index) in listItem" :key="index" @click="itemClick(item.id)">
						<!-- 头像 名字栏 -->
						<view class="item-one">
							<view class="item-one-left">
								<image src="../../static/dingdan2.png" class="item-one-img"></image>
								<text class="item-one-tittle">{{item.tittle}}</text>
							</view>
							<view class="item-one-sign" style="color: #c3c2a7;" v-if="item.status==1">待审核</view>
							<view class="item-one-sign" style="color: #b3dfbc;" v-if="item.status==2">进行中</view>
							<view class="item-one-sign" style="color: #ECAD9E;" v-if="item.status==3">已结束</view>
						</view>
						<!-- 其他信息 -->
...
export default {
	...
	data() {
		return {
			...
			currentPage:'index',
			toView:'',//回到顶部id
			tabTitle:['全部','待审批','进行中','已结束'], //导航栏格式 --导航栏组件
	...
	methods: {
		...
		itemClick(id){
			uni.navigateTo({
				url:"../job-detail/job-detail?id="+id
			})
		},
		toTop(){
			this.toView = ''
